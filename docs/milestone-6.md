...
**C86 — renderer-triggered summarisation instead of main-emitted `conversations:summaryRequired` event.** Plan said `ConversationRepository.append` would detect the cap-crossing in main + emit a `summary-required` event to the renderer (the M4 `connections:changed` pattern). Built differently: the renderer's `ChatView` watches `messages.length` against `SUMMARY_TRIGGER_THRESHOLD` (180, set below the M5 hard cap of 200 to give the operator a 20-message runway) and fires `summariseAndTrim` itself. Why: the summariser call needs the renderer-resident `LlmProvider` (M5 D4 — provider runs in renderer; the API key lives in the credential vault) + the renderer already owns the `messages.length` invariant via the Zustand store. Routing through main would add a round-trip without adding correctness. The main-side hard-cap silent-drop in `ConversationRepository.append` stays as a safety net — if the renderer never fires for any reason (renderer crash, summary-trigger threshold bug), the conversation still doesn't grow unbounded.

**C86 — `summariseAndTrim` accepts the summariser as a callback rather than constructing the provider itself.** Keeps the conversations Zustand store free of `LlmProvider` / IPC dependencies — the unit tests inject a stubbed `summarise(headSlice) → Promise<SummariserResult | null>` directly. The provider + model resolution + signal threading lives in `ChatView` where the chat-runner already orchestrates those concerns.

**C86 edge cases (per promt19):**
- **Failure (#1)** — `runSummariser` returns `null` on any non-success path (provider throw, error event, empty/whitespace summary, aborted). `summariseAndTrim` then drops the head slice silently + sets `outcome: 'dropped'`. `ChatView` surfaces a fallback chip in the header ("Summary unavailable — older messages dropped"); cleared when the next successful trim lands.
- **Cancellation (#2)** — `AbortSignal` threads through `runSummariser → provider.streamResponse`. The chat view's `useEffect` cleanup function aborts on conversation switch / unmount so a stale summary call doesn't write back into the new conversation's store slot.
- **Prompt design (#3)** — `SUMMARISER_SYSTEM_PROMPT` instructs the model to preserve **key facts / tool results / conclusions reached / pending questions**; first-person; ≤200 tokens. The head slice threads full `tool_use` / `tool_result` blocks through (not just text); diagnostic value lives in tool results. `Plugin.summarisationHints?` deferred to `docs/m6-followups.md` as the per-plugin-guidance seam.
- **Re-summarisation continuity (#4)** — `computeHeadSlice` consumes any prior `marker: 'summary'` message into the next head slice (prior summary + the next 100 messages). The single summary marker grows in scope across multiple trim cycles rather than accumulating multiple markers in the conversation log.
- **Schema (#5)** — additive: `WorkspaceLlmSettings.summariserModel?: 'haiku' | 'sonnet' | 'opus' | 'same-as-main'` (default `'haiku'`); `Message.marker` enum already widened in C83b to include `'summary'`. No version bump. The migrator's permissive `parseLlmSettings` keeps M5 workspaces (without the field) loading.

**C86 — cost transparency.** The summariser call's `usage` lands on the synthetic summary marker's `message.usage` field; the UsageBadge picks it up via the same `sumUsage` aggregate the M5 path uses, so workspace-global session spend totals see the cost without a separate code path.

**C86 split judgement** — landed as a single cohesive commit (the summariser module + the trim transaction + the collapsible-card rendering + the workspace setting + the ChatView trigger). The C85 split rule from promt18 was specific to that commit's orthogonal cache infrastructure; C86's surfaces are all part of the one summarise-then-drop feature, so an intermediate split would leave the runner / store / UI in inconsistent shape mid-tree.

**C87 — `useHostBus.selectedOrd` as a state channel, not an event channel.** The M5 C79 `pendingOrdNav` channel is event-shaped (publish → peek/consume; one-shot trigger). The C87 selection channel is state-shaped: publish overwrites, peek is the read, no consume. Why: selection is a continuous observable (the chat empty-state's button labels need to track it at render time), not a one-shot trigger to handle. The bus's docstring now distinguishes the two shapes explicitly. The publisher (Niagara `ExplorerView`) has a structural-equality short-circuit so the upstream useEffect doesn't re-publish identical values on every render → no subscribed-component re-render cascade.

**C87 — selection plumbing as one bus channel, not per-plugin namespacing.** The recon mentioned `peekSelectedOrd()` as a single accessor; concrete shape lands as a single `selectedOrd: {ord, displayName?} | null` field. Multi-plugin namespacing (`selectionByPlugin: Record<pluginName, ...>`) is m6-followup work — surfaces when the second selection-aware plugin appears. M6 ships single-plugin Niagara; the simpler shape doesn't pre-pay the cost.

**C87 — `useExplorerStore.known` contract formalisation lives in CONTRIBUTING.md, not its own doc file.** The m4-followup (3-consumer threshold) said "Surface in CONTRIBUTING.md alongside the M3 Zustand-singleton lesson"; C87 followed that exactly. A separate doc would have fragmented the canonical-patterns location.

**C87 — no e2e for selection-aware label decoration.** The recon mentioned extending `chat-rooftop.spec.ts` for selection verification; the C87 unit tests on `host-bus` (6 tests) + `host-selection` (7 tests) cover the contract. A dedicated selection e2e is m6-followup if regression risk shows.

**C88 — chat-plan.spec.ts caught a real bug.** The C83b plan-step-complete handler used `[currentStepId!]: {status: 'done'}` inside a setPendingPlan updater, but the line below cleared `currentStepId = null` BEFORE React's batched update ran the closure — so the updater wrote `[null]: ...` and the real step never transitioned out of 'running'. The chat-plan e2e's "4/5 done · 1 skipped · 0 failed" assertion is exactly the kind of post-completion summary check that catches this class of bug; the fix is a const-snapshot before the clear. (Lesson: any time a closure inside a state updater reads a mutable variable that changes on the next statement, snapshot first. Generalises to React's batched-update + captured-let composition. Adding to the canonical pitfalls if it shows up a second time.)

**C88 — test escape hatch via `globalThis` tuning override rather than a test-only IPC channel or env var.** The chat-summary e2e needs to trigger summarisation without bloating to 180 messages. Three approaches considered: (1) test-only IPC channel + force-reload on the conversations store, (2) renderer-side env var read at module-load time, (3) `globalThis.__MCPSTUDIO_E2E_SUMMARY_TUNING` read at runtime by `getTriggerThreshold()` / `getHeadSliceCount()`. Picked (3) — the override is invisible to production (the global is undefined; getters return module defaults), set via Playwright's `addInitScript` before renderer JS runs, and doesn't require an env var to plumb through Electron's main → renderer boundary. The pattern generalises for any future "thresholds that production can't easily replay in e2e".

**C88 — chat-summary uses a sentinel-in-user-message to route the mock summariser to its failure program**, not a separate "failure mode" IPC channel. The mock LLM provider's matcher inspects `req.messages` for the literal `SUMMARY_FAILURE_SENTINEL` string; if present, routes to the error-event program. Keeps the failure path inside the mock infrastructure (no new test channels) and exercises exactly the same code path real failure would hit (provider returns `error` event → `runSummariser` returns null → `summariseAndTrim` does silent-drop + outcome:'dropped').

**Phase D — no per-step component-level tests for PlanEditor.** The recon mentioned ~5 component tests for PlanEditor. The desktop app doesn't have `@testing-library/react` installed; introducing it for one feature was rejected as scope creep — the C88 e2e (chat-plan.spec.ts) covers the rendered behaviour end-to-end (step list, runIf hint, summary counts). Adding RTL is m7-followup if a richer interactive component lands that benefits from unit-level coverage.
