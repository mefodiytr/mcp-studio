...
| `m4-history.png` | M4 / Phase B — the Niagara **History** view: range presets (1 h / 4 h / 1 d / 7 d / 30 d / Custom), aggregation toggle, multi-history series chips with "+ Add series" (over the Explorer's known cache), the `<TimeSeriesChart>` rendering a 1000-row sine and the per-series paginated table dual view below. |
| `m4-monitor.png` | M4 / Phase C — the Niagara **live Monitor**: drag-from-Explorer to add; two watch rows with sparklines, per-row poll interval popovers, threshold low/high inputs. The Drivers row's value cell is red because its current reading crossed the `high: 0` threshold (the §C66 cross-visual). |
| `m4-perf.png` | M4 / Phase D — the **Performance** view: log-ish latency histogram (BarChart), recent-vs-previous p95 regression panel ("Stable" / "no prior window yet" when the prior window is empty), and the slowest-calls table. Window selector + scope picker in the header. |
| `m5-chat-empty-state.png` | M5 / Phase A — the **Assistant** rail item's empty state on a Niagara connection: MOCK PROVIDER badge (e2e env-gated mock); the "Diagnostic flows" row with the Niagara plugin's two flow buttons (Rooftop diagnosis / Knowledge layer summary); the four starter-question chips contributed by the plugin. |
| `m5-rooftop-diagnosis.png` | M5 / Phase C — the **Rooftop diagnostic flow** mid-walk: the launcher dialog filled with "rooftop unit 5", three collapsible ToolCallEnvelopes (findEquipment / inspectComponent / getActiveAlarms) ordered by the ReAct loop, and the final assistant message with the inline chart code-fence rendered as a `<TimeSeriesChart>`. |
| `m5-chart-inline-rendering.png` | M5 / Phase C — focus on the **chart-inline** render: the chart card (border, optional title bar, recharts SVG with palette colour, "Show payload" debug toggle at the bottom). The `<TimeSeriesChart>` primitive is the same one M4's History view + Performance view use — single recharts shared chunk, the M4 manualChunks win continues. |
| `m5-ai-proposed-write-in-queue.png` | M5 / Phase C — the **Changes view** after an AI-attributed write was intercepted by the C75 safety boundary: one pending `setSlot` op badged **Reversible** + the blue **AI** chip (Bot icon) with a tooltip carrying the originating conversation id. Apply by the operator sends `caller` absent (back-compat `'human'`); the M3 dispatch path is unchanged. |
| `m5-chat-stopped.png` | M5 / Phase C — chat conversation captured mid-cancel: the `'cancel'` mock-program emits a slow text stream paced by `__delay` sentinels; the Stop button fires after the first delta lands; the conversation records a synthetic `— stopped by user —` marker; the Send button returns for the next turn. |
