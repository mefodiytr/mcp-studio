...
### D8 — Chart-inline rendering: **`<chart>` tag in assistant markdown, parsed by a custom `react-markdown` extension, rendered via `@mcp-studio/charts`**

Three shapes:

- (a) **Plain markdown image (`![chart](data:image/png;base64,…)`)** — would need server-rendered PNG. Big infra detour for a desktop app.
- (b) **Structured `<chart>` tag in the assistant's text** — `<chart type="timeseries" data="<base64-encoded-json>" />`. The chat renderer parses it via a `react-markdown` plugin, swaps in a `<TimeSeriesChart>` from `packages/charts`. JSON serialisation is small because the LLM is instructed to downsample to ≤2k points before emission.
...
**Recommendation: (b).** The LLM is instructed (system prompt — host-base, not plugin-contributed) that to embed a chart, emit a `<chart>` tag:

```
<chart type="timeseries" title="SAT trend (last 24 h)" data="<base64>" />
```

The base64-decoded payload is `{ series: [{ name, points: [{t,v},…] }, …], xDomain?, yDomain? }`. The renderer's `react-markdown` extension routes `<chart>` to a `<ChatChart>` component (new — `apps/desktop/src/renderer/src/features/chat/ChatChart.tsx`) that decodes + validates with zod, then renders `<TimeSeriesChart>` from `@mcp-studio/charts`. **Defensive parse failure** renders the raw `<chart>` tag's contents with a `(invalid chart payload)` tag — the conversation doesn't break.

The mock LLM provider for tests emits deterministic `<chart>` blocks in the rooftop scenario so the e2e can assert "a `TimeSeriesChart` SVG renders inside a chat message".

**Why base64 inside the tag:** Anthropic streams text in chunks at arbitrary boundaries; embedding raw JSON with quotes / newlines makes the chunk-boundary parsing fiddly. Base64 keeps the payload a single opaque blob; the parser waits for the closing `/>` then decodes.

**Future extensions** (m5-followups, all behind the same parser): `<chart type="bar" …>` (`BarChart`), `<sparkline data="…">` inline in a sentence ("the supply temp rose <sparkline data='…'> over the last hour"), `<ord-card ord="…">` (a quick-look card with name + parent + last reading — composable with the rest of Niagara's read tools).
...
- **C75 — `feat(desktop): caller-attributed tool dispatch + AI-write safety boundary (main)`.** Thread `caller: 'human' | { type:'ai', conversationId, agentId? }` through the `tools:call` IPC + `ConnectionManager.callTool`. **Default to `'human'`** if the caller is omitted (back-compat for the M1+M2+M3 paths). The `isWriteCall(effectiveAnnotations)` predicate (already in renderer M3) gets a main-side twin in `apps/desktop/src/main/agent/intercept.ts`. If `caller.type === 'ai'` AND `isWriteCall` → instead of dispatching to the MCP SDK, return `{ kind: 'pendingEnqueued', op: { name, args }, attribution: caller }`. The `tool-history-repository` audit-log entry gains the optional `actor?: 'human' | {type:'ai',conversationId}` field. Renderer: `useConversationStore` listens for `pendingEnqueued` outcomes; broadcasts `host.broadcast({type:'plugin:enqueue', op, attribution})`; the Niagara plugin's `pending-store.enqueue` adds the `source` field on each op; the Changes view badges AI-proposed ops with an "AI" chip + a deep-link back to the originating conversation. + ~10 unit tests (the interceptor; the audit field; the renderer-side bus subscriber). **AC:** the ReAct runner can call read tools normally; if the LLM (or the mock) proposes a write tool, the call is intercepted in main, never reaches the MCP server, and lands in the Niagara plugin's pending-changes queue with AI attribution; the Changes view shows it badged; the operator can Apply (sends `caller:'human'` this time, which routes normally).
- **C76 — `feat(desktop): chat-inline chart rendering (<chart> parser + ChatChart)`.** Custom `react-markdown` extension for `<chart type="…" title="…" data="<base64>" />` → `<ChatChart>` component. `ChatChart` decodes + zod-validates (`chartPayloadSchema`) → renders `<TimeSeriesChart>` from `@mcp-studio/charts` inside a fixed-aspect container. Defensive fallback for malformed payloads. + parser unit tests (round-trip; corruption; oversized payload guard at 256 kB). + a "Show payload" toggle revealing the decoded JSON for debugging. + the host system prompt is updated with a `<chart>` usage hint (composed by the host before plugin contributions). **AC:** the mock provider's rooftop program emits a `<chart>` block; the chat view renders a `TimeSeriesChart` inside the assistant message; the chart is keyboard-focusable for screen readers.
...
3. **The `<chart>` parser collides with raw markdown** (e.g. the LLM emits `<chart>` inside a code block; the parser must not rewrite inside code blocks). One-spot fix in the `react-markdown` extension; ad-hoc if it surfaces.
